...

{
	...
	$short_results = mysql_query('SELECT id FROM ' . DB_TABLE. ' WHERE long_url="' . mysql_real_escape_string($url_to_shorten) . '"');
	if (mysql_num_rows($short_results) > 0) { 

	$already_shortened = mysql_result($short_results, 0, 0);
