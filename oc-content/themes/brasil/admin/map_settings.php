...
<h2 class="render-title <?php echo (osc_get_preference('footer_link', 'brasil') ? '' : 'separate-top'); ?>"><?php _e('Map Options', 'brasil'); ?></h2>
<div class="flashmessage flashmessage-warning flashmessage-inline"><p><strong><?php _e('How to assign a region to your map?','brasil'); ?></strong><br><?php _e('First, click in the map to select the area you want to set. Then, a dialog will appear and you only need to select the region you want to assign. Finally, once you have selected the region, click the save button.','brasil'); ?></p></div>
...
function countrySelect($id = null,$selected = null){
    ...
    $echo .= '<option value="">' . __('Select a region', 'brasil') . '</option>';
...
$regions = json_decode(osc_get_preference('region_maps','brasil'),true);
for($i=0;$i<27;$i++){
    ?>
    <form id="region-<?php echo $i; ?>" class="region-dialog has-form-actions" onsubmit="return validateForm('region-<?php echo $i; ?>')" >
        <div class="form-horizontal">
            ...
            <div style="display:none" class="form-row error">
                <div>*<?php _e('Select a region first', 'brasil'); ?></div>
            </div>
